...
load_dotenv()
DIALECT_MODEL = os.getenv("DIALECT_MODEL", "badrex/mms-300m-arabic-dialect-identifier")
...
def slice_audio(input_path: str, start_time: str | float, duration: float) -> str:
    ...
    if Path(output_path).stat().st_size == 0:
        raise ValueError("Sliced audio file is empty. Check start time and duration.")

    return output_path


@st.cache_resource
def load_whisper_model(device: str) -> Any:
    """
    ...
    """
    model_id = WHISPER_MODEL if WHISPER_MODEL else "turbo"
    return whisper.load_model(name=model_id, device=device)


def detect_language(model: Any, audio_path: str) -> str:
    """
    Detect the language of the audio file using Whisper.

    Args:
        model (Any): The loaded Whisper model.
...
def load_classifier(device: str) -> Pipeline:
    ...
    """
    model_id = (
        DIALECT_MODEL if DIALECT_MODEL else "badrex/mms-300m-arabic-dialect-identifier"
    )
    return pipeline(
...
def predict_dialect(classifier: Pipeline, file: str) -> list[dict[str, Any]]:
    """
    Detect the dialect of the given audio file.

    Args:
        classifier (Pipeline): The audio classification pipeline.
        file (str): Path to the audio file.

    Returns:
        list[dict[str, Any]]: The classification predictions.
    """
    predictions = classifier(file)
    ...
    return [
        dict(prediction) for prediction in predictions if isinstance(prediction, dict)
    ]
